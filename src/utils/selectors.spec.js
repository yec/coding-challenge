import { programTypeSelector } from './selectors';

describe('Selectors', () => {
  it('should filter by series', () => {
    const selector = programTypeSelector('series');
    const filtered = selector(getFeed());
    expect(filtered).toEqual(getExpectedSeries());
    expect(filtered.length).toEqual(21);
  });

  it('should return 21 items', () => {
    const selector = programTypeSelector('series');
    const filtered = selector(getFeed());
    expect(filtered.length).toEqual(21);
  });
  ...
  it('should filter by movie', () => {
    const selector = programTypeSelector('movie');
    const filtered = selector(getFeed());
    expect(filtered).toEqual(getExpectedMovies());
    expect(filtered.length).toEqual(21);
  });
});

const getExpectedSeries = () => {
  return [
    {
      "title": "11.22.63",
